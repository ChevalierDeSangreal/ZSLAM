...
class EnvMove():
    ...
    def update_acc_attacc(self):
        ...
        2. obstacle_opp_vector，模长为 sqrt(2 * 0.5 * max_acc * avoid_radius)
        """
        max_acc = torch.tensor(self.cfg.agent_cfg.max_acc, device=self.device)
        avoid_radius = self.cfg.agent_cfg.avoid_radius
        ...
        # 计算避障加速度
        acc_magnitude_obstacle = torch.sqrt(2 * max_acc * 1 * avoid_radius)
        avoid_obstacle_vector = normalized_obstacle_opp_vector * acc_magnitude_obstacle  # (B, 2)

        # 在智能体到障碍物距离大于 avoid_radius 时，将避障加速度设为 0
        mask_safe = obstacle_distance > avoid_radius
    ...
    def get_done(self, action):
        ...
        3. 碰撞
        返回:
            done_mask: shape (batch_size,) 的布尔 tensor，指示哪些 agent 已终止
            done_idx: shape (num_done,) 的 tensor，指示哪些 agent 已终止
        ...
        # 综合终止条件
        done_mask = mask_collision | mask_timeout | mask_reached
        done_idx = torch.nonzero(done_mask, as_tuple=False).squeeze(1)
        ...
        return done_mask, done_idx, info
    ...
    def step(self, action):
        ...
        mask_reset, idx_reset, info = self.get_done(action)
        ...
        step_output["reward"] = reward
        step_output["done"] = mask_reset
