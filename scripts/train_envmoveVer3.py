...
"""
相比于train_envmoveVer3.py
使用了新的模型ZSLAModelVer4
使预训练embedding与下游任务兼容
"""
...
def get_args():
	...
	# model setting
	parser.add_argument("--param_save_name", type=str, default='movingVer3.pth', help="The path to save model parameters")
	parser.add_argument("--param_load_name", type=str, default='movingVer3.pth', help="The path to load model parameters")
...
if __name__ == "__main__":
	...
	model = ZSLAModelVer4(image_dim=64, hidden_dim=256, query_num=10, num_classes=2, device=device, is_pretrain=True)
	...
	for epoch in range(args.num_epoch):
		...
		for step in range(args.len_sample):
			...
			if step % 10 == 0:
				...
				inputs = {}
				inputs["image"] = image
				inputs["agent_pos_encode"] = agent_pos
				inputs["h_local"] = h_local
				inputs["h_global"] = h_global
				inputs["local_query"] = gt["local_query_encode"].detach()
				inputs["global_query"] = gt["global_query_encode"].detach()
				...
				output_local_distance, output_local_class, output_global_exprate, h_local, h_global = model(inputs)
