...
def get_args():
	...
	parser.add_argument("--len_sample", type=int, default=8, help="Length of a sample")
...
if __name__ == "__main__":
	...
	envs = EnvMove(batch_size=args.batch_size, device=args.device, dtype=torch.float16)
	pos_encode_dim = 16

	model = ZSLAModelVer2_pos_test(
		image_dim=512, 
		img_feature_dim=8,
	...
	for epoch in range(args.num_epoch):
		...
		for step in range(args.len_sample):
			...
			image = step_output["image"].detach().to(torch.float32)
			agent_pos = step_output["agent_pos_encode"].detach().to(torch.float32)
			gt = step_output["gt"]
			idx_reset = step_output["idx_reset"]
        
			output_local_distance, output_local_class, output_global_exprate = model(image, agent_pos, gt["local_query_encode"].detach().to(torch.float32), gt["global_query_encode"].detach().to(torch.float32))
			...
			loss_local_distance = criterion_mse(output_local_distance, gt["local_gt_distance"].detach().to(torch.float32))
			...
			loss_global_exprate = criterion_mse(output_global_exprate, gt["global_explrate"].detach().to(torch.float32))
